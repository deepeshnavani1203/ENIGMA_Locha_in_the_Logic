...
const CompanyDashboard: React.FC = () => {
    ...
    const totalContributed = campaigns.reduce((acc, c) => acc + c.raised, 0);
    const totalInitiatives = campaigns.length;

    return (
        <div className="space-y-8 animate-fade-in">
             <div>
                <h1 className="text-3xl font-bold font-display text-text-primary">Company Dashboard</h1>
                <p className="mt-2 text-lg text-text-secondary">
                    Welcome, <span className="font-semibold text-primary">{user?.fullName}</span>! Manage your CSR initiatives.
                </p>
            </div>

            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
                <StatCard title="Total Initiatives" value={totalInitiatives.toString()} icon="briefcase-outline" color="text-primary" />
                <StatCard title="Total Contributed" value={`$${totalContributed.toLocaleString()}`} icon="trending-up-outline" color="text-secondary" />
            </div>
            ...
            <div className="bg-surface rounded-xl shadow-serene border border-border">
                <div className="p-4 border-b border-border md:flex md:items-center md:justify-between">
                    <h2 className="text-xl font-bold font-display text-text-primary">Our CSR Campaigns</h2>
                     ...
                     {isLoading ? (
                        ...
                        <table className="min-w-full text-sm text-left text-text-primary">
                            <thead className="bg-gray-50 text-xs text-text-secondary uppercase">
                                <tr>
                                    <th scope="col" className="px-6 py-3">Initiative Title</th>
                         ...
                         <p className="p-6 text-center text-text-secondary">Your company has not started any initiatives yet.</p>
                    )}
                </div>
            </div>
        </div>
